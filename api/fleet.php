...
try {
    if ($method === 'GET') {
        $stmt = $pdo->query("SELECT id, vehicle_name, plate, rent_cost, status, created_at FROM vehicles ORDER BY id DESC");
        ...
        $sql = 'INSERT INTO vehicles (vehicle_name, plate, rent_cost, status) VALUES (?, ?, ?, ?)';
        $values = [trim($data['vehicle_name']), trim($data['plate']), $data['rent_cost'] ?? 0, $data['status'] ?? 'available'];
        ...
        $pdo->prepare("DELETE FROM vehicles WHERE id = ?")->execute([$data['id']]);
